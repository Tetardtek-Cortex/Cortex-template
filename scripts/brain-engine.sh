...
check_prereqs() {
  ...
  if ! command -v python3 &>/dev/null; then
    err "python3 non trouvé — sudo apt install python3 python3-pip python3-venv"
    exit 1
  fi

  # Créer et activer le venv (recréer si cassé)
  local venv_dir="$ENGINE_DIR/.venv"
  if [[ ! -f "$venv_dir/bin/activate" ]]; then
    if ! python3 -m venv --help &>/dev/null; then
      local py_ver
      py_ver=$(python3 --version 2>&1 | awk '{print $2}' | cut -d. -f1-2)
      err "python3-venv manquant — sudo apt install python${py_ver}-venv"
      exit 1
    fi
    rm -rf "$venv_dir"
  ...
  if ! python3 -c "import fastapi, uvicorn" 2>/dev/null; then
    ...
    pip install -q -r "$ENGINE_DIR/requirements.txt" || {
      err "pip install a échoué — vérifier $ENGINE_DIR/requirements.txt"
      exit 1
    }
...
cmd_start() {
  ...
  # Build brain-ui si dist/ absent et npm disponible
  local ui_dir="$BRAIN_ROOT/brain-ui"
  if [[ -f "$ui_dir/package.json" && ! -d "$ui_dir/dist" ]]; then
    if command -v npm &>/dev/null; then
      info "Build brain-ui (dashboard)..."
      (cd "$ui_dir" && npm install --silent 2>/dev/null && npm run build --silent 2>/dev/null) \
        && ok "Dashboard buildé" \
        || warn "Build brain-ui échoué — dashboard indisponible"
    else
      warn "npm absent — dashboard indisponible (installer Node.js pour le dashboard)"
    fi
  fi
