...
if [[ ! -f "$DB_PATH" ]]; then
    SCHEMA="$BRAIN_ROOT/brain-engine/schema.sql"
    if [[ -f "$SCHEMA" ]]; then
        python3 -c "
...
conn.close()
" 2>/dev/null && echo "✅ brain.db initialisé" >&2
    else
        # Minimal claims table
        python3 -c "
import sqlite3
conn = sqlite3.connect('$DB_PATH')
conn.execute('''CREATE TABLE IF NOT EXISTS claims (
    sess_id TEXT PRIMARY KEY, scope TEXT, type TEXT, zone TEXT,
    mode TEXT, project TEXT, status TEXT DEFAULT 'open',
    opened_at TEXT, closed_at TEXT, result TEXT,
    duration_min REAL, energy INTEGER, intention TEXT, tags TEXT, deliverables TEXT
)''')
conn.commit(); conn.close()
" 2>/dev/null && echo "✅ brain.db initialisé (minimal)" >&2
    fi
fi
...
" 2>/dev/null | while IFS=: read -r _ name host; do
        echo ""
        echo "💻 $name ($host)"
        result=$(ssh -o BatchMode=yes -o ConnectTimeout=3 "$(whoami)@$host" \
