...
Fenêtre 2 (travail terrain)     → écrit du code dans mon-projet/
...
sess_id:          sess-20260317-1000-mon-projet-auth
type:             satellite
scope:            mon-projet/src/auth/    ← périmètre exclusif de cette instance
...
git commit -m "bsi: open satellite sess-20260317-1000-mon-projet-auth"
git push
```
...
Instance 2 : scope mon-projet/ → code, tests, deploy
```
...
```
Instance A : scope mon-projet/src/auth/     → JWT refacto
Instance B : scope mon-projet/src/api/      → endpoints REST
```
...
```yaml
mode: rendering
scope: mon-projet/      ← seul périmètre autorisé
```
