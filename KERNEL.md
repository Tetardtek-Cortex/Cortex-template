...
Déclaration dans le claim pilote :
  mode: rendering
  scope: mon-projet/        ← le seul périmètre autorisé
```
...
**Pourquoi human-only :**
Laisser des satellites écrire en zone kernel sans garde-fou = dérive garantie. Toute promotion vers l'écriture autonome nécessite un orchestrator mature et auditable.
...
kerneluser: false →  identityShow: off (défaut client — mode clean/pro)
                     ...
                     tier : selon clé API (brain-key-server)
```
