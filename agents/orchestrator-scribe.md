...
Ou directement :
```
orchestrator-scribe, y'a-t-il des signaux pour mybrain@desktop ?
orchestrator-scribe, envoie un signal READY_FOR_REVIEW à review@laptop sur agents/security.md
orchestrator-scribe, je passe la main à mybrain@laptop — HANDOFF depuis agents/vps.md section ## Patterns
```
...
```
mybrain@desktop  →  travaille sur <fichier>
                  →  ferme claim
...
review@laptop    →  reçoit signal au démarrage
                  ...
                  →  ferme claim
                  →  signal REVIEWED → mybrain@desktop

mybrain@desktop  →  reçoit REVIEWED
                  →  lit reviews/
...
```
mybrain@desktop  →  travaille jusqu'à un point d'arrêt naturel
                  →  signal HANDOFF → mybrain@laptop avec payload : "reprendre à ## Section X"

mybrain@laptop   →  reçoit HANDOFF
