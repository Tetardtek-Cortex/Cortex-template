...
10. Output ≤ 6 lignes :
    ...
    <brain_name>@<machine> [<tier>] — boot mode: <type>[/<project>]
...
| `"GDD"`, `"vision"`, `"design doc"`, `"rédige"` sans code attendu | `brain-write` | Agent documentaire (`doc`) | Rédactionnel — validation livrable avant commit |
...
4. Étape 3 — brain-compose.local.yml
   Générer brain-compose.local.yml (auto-créé par brain-setup.sh, ou manuellement)
...
|-------------------------------|--------------|
| Nom de projet explicite (`my-saas-app`, `my-website`…) | Auto — charge `projets/X.md` + agent métier |
...
```
User dit "brain boot mode work/mon-projet" en session navigate :
  1. Close claim navigate (minimal : metabolism-scribe → BSI close)
  2. Exécuter BHP complet pour session-work (nouveau claim)
  3. Output : "↑ Navigate → Work/mon-projet — claim <new-id> ouvert"
```
