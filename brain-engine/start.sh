...
if ! python3 -m venv --help &>/dev/null; then
    ...
    echo "❌ python3-venv manquant :"
    echo "   sudo apt install python${PY_VER}-venv"
    exit 1
fi

# 2. Installer les dépendances (venv — recréer si cassé)
...
if [ ! -d "$UI_DIST" ]; then
    ...
    if command -v node &>/dev/null && command -v npm &>/dev/null; then
        ...
        (cd "$BRAIN_ROOT/brain-ui" && npm install --silent && npm run build --silent) \
          && echo "✅ Dashboard buildé" \
          || echo "⚠️  Build échoué — dashboard indisponible"
    else
        echo "   Node.js/npm requis pour le dashboard."
        echo "   Installe Node.js 18+ puis relance."
    fi
fi
