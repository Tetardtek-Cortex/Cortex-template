...
> Pour le cycle de vie detaille : `wiki/session-lifecycle.md`
...
```
"Cette action depasse le scope navigate — brain boot mode work/mon-projet pour continuer."
```
...
3. Brain update    → scribe met a jour focus, projets, agents si changement
4. Coach           → rapport de session (sauf en navigate, deploy, infra, urgence, audit)
5. BSI close       → le claim est ferme, la session est tracee
```
...
> 🟢 **free** — 6 sessions (navigate, work, debug, brainstorm, brain, handoff). 19 agents. Pas de cle API. Le brain fonctionne quand meme.

> 🔵 **featured** — 8 sessions (+coach, capital). 26 agents. Progression personnelle, RAG, coaching complet, documentation, content writing.

> 🟠 **pro** — 14 sessions (+audit, deploy, infra, urgence, refacto, migration). 48 agents. Tous les agents metier : code-review, security, optimizer, vps, ci-cd, database-architect, api-designer.

> 🟣 **full** — Toutes les sessions (+kernel, edit-brain, pilote). 56 agents (tous). Acces kernel complet, owner du brain.
...
4. Ajouter la zone access dans `KERNEL.md`
5. Mettre a jour `wiki/session-lifecycle.md`

### Comment escalader depuis navigate ?

Dis simplement `brain boot mode <type>` (ex: `brain boot mode work/mon-projet`). Le brain ferme navigate et ouvre la session demandee. Tu peux aussi decrire ce que tu veux faire — le brain detectera le debordement et proposera le bon type.
...
- **Patterns** : `wiki/patterns.md` — patterns recurrents documentes
- **Vocabulaire** : `wiki/vocabulary.md` — lexique brain
